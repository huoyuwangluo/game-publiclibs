...
var tips;
(function (tips) {
    var TradingSellShowItemTip = (function (_super) {
        __extends(TradingSellShowItemTip, _super);
        function TradingSellShowItemTip() {
            return _super.call(this) || this;
        }
        Object.defineProperty(TradingSellShowItemTip, "instance", {
            get: function () {
                if (!TradingSellShowItemTip._instance) {
                    TradingSellShowItemTip._instance = new TradingSellShowItemTip();
                    TradingSellShowItemTip._instance.touchEnabled = TradingSellShowItemTip._instance.touchChildren = false;
                }
                return TradingSellShowItemTip._instance;
            },
            enumerable: true,
            configurable: true
        });
        TradingSellShowItemTip.prototype.show = function (items, handler) {
            this.labTitle.text = Language.J_GXHDKPMJL;
            this._items = [];
            if (items.length > 3) {
                for (var i = 0; i < 3; i++) {
            ...
            this.y = mg.stageManager.stageHeight - 580;
            this.scaleX = this.scaleY = 1.4;
            this.alpha = .5;
            egret.Tween.get(this).to({ scaleX: 1, scaleY: 1, alpha: 1 }, 500, utils.Ease.quadInOut).wait(1500).call(function () {
                this.hide();
                if (handler) {
                    handler.run();
                }
        ...
        TradingSellShowItemTip.prototype.hide = function () {
            if (this._items) {
                var icon = void 0;
        ...
        TradingSellShowItemTip.prototype.flyOverHandler = function (icon) {
            if (icon.parent) {
        ...
        //根据长度显示多少个装备
        TradingSellShowItemTip.prototype.showIconPosition = function (num) {
            for (var i = 0; i < 3; i++) {
                if (i < num) {
        ...
        return TradingSellShowItemTip;
    }(ui.GetSpuerEquitmentSkin));
    tips.TradingSellShowItemTip = TradingSellShowItemTip;
    __reflect(TradingSellShowItemTip.prototype, "tips.TradingSellShowItemTip");
})(tips || (tips = {}));
